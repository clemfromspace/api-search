...
const Hit = ({hit}) => {
  return (
    <div key={hit.objectID} className="bg-white p-8 rounded shadow hover:shadow-md my-2 transition">
          ...
          <div className="flex flex-col h-screen font-sans">
            ...
            <div className="flex flex-grow">
              <aside className="w-64 flex-no-shrink bg-white border-grey-light border-solid border-r z-10 max-h-screen hidden md:block overflow-hidden">
                <div className="h-full overflow-y-auto">
                  <h4 className="font-normal uppercase text-xs tracking-wide text-grey-dark p-4 border-grey-light border-solid">Categories</h4>
                  <Menu attribute="Category" limit={8} searchable={true}/>
                  <h4 className="font-normal uppercase text-xs tracking-wide text-grey-dark px-4 pt-4 pb-1 border-grey-light border-solid border-t">Features</h4>
                  <div className="py-2">
                    <ToggleRefinement attribute="HTTPS" label="HTTPS" value={true}/>
                    <ToggleRefinement attribute="Cors" label="CORS" value="yes"/>
                  </div>
                  <h4 className="font-normal uppercase text-xs tracking-wide text-grey-dark px-4 pt-4 pb-1 border-grey-light border-solid border-t">Authentication</h4>
            ...
            <footer className="flex flex-no-shrink justify-between flex-col sm:flex-row p-4 border-grey-light border-solid border-t text-sm text-grey-dark">
              <div className="text-center sm:text-left">
                Made by <a href="https://twitter.com/@frontstuff_io" className="text-blue hover:text-blue-darker transition no-underline" target="_blank" rel="noopener noreferrer">@frontstuff_io</a> and <a href="https://twitter.com/@cdenoix" className="text-blue hover:text-blue-darker transition no-underline" target="_blank" rel="noopener noreferrer">@cdenoix</a>
              </div>
              <div className="text-center sm:text-right mt-2 sm:mt-0">
                Data from <a href="https://api.publicapis.org/" className="text-blue hover:text-blue-darker transition no-underline" target="_blank" rel="noopener noreferrer">api.publicapis.org</a>
              </div>
            </footer>
          </div>
        </InstantSearch>
